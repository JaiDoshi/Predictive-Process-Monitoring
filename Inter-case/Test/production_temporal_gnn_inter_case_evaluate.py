...
#Read dataframe
df = pd.read_csv("/content/drive/My Drive/Process Modelling Datasets/Production.csv", sep=';')
...
processAttributeNames = ['Work_Order_Qty']

one_hot = pd.get_dummies(df['Part_Desc_'])

df = df.drop('Part_Desc_',axis = 1)
one_hot = one_hot.add_suffix('_r1')
processAttributeNames += list(one_hot.columns)
df = df.join(one_hot)

one_hot = pd.get_dummies(df['Rework'])

df = df.drop('Rework',axis = 1)
...
one_hot = pd.get_dummies(df['Report_Type'])

df = df.drop('Report_Type',axis = 1)
one_hot = one_hot.add_suffix('_r4')
df = df.join(one_hot)

one_hot = pd.get_dummies(df['Resource.1'])

df = df.drop('Resource.1',axis = 1)
...
for column in df:
    if column == 'timesincelastevent' or column == 'timesincecasestart' or column == 'timesincemidnight' or column == 'event_nr' or column == 'open_cases' or column == 'Work_Order_Qty' or column == 'Qty_Completed' or column == 'Qty_for_MRB' or column == 'activity_duration':
        mex = df[column].max()
        mine = df[column].min()
        df[column] -= mine
        df[column]/=(mex-mine)
