...
df = pd.read_csv("Process Modelling Datasets/BPIC11_f1.csv", sep=';')

#Assert that there are no NaNs for any activity
assert(df['Activity code'].isnull().values.any() == False)

df['Activity code'] = pd.Categorical(df['Activity code'])
activityNames = list(df['Activity code'].cat.categories)
numActivities = len(activityNames)
df['Activity code'] = df['Activity code'].cat.codes
...
#Drop some columns
df = df.drop('Diagnosis Treatment Combination ID', axis=1)
...
one_hot = pd.get_dummies(df['Treatment code'])

df = df.drop('Treatment code',axis = 1)
one_hot = one_hot.add_suffix('_r2')
processAttributeNames += list(one_hot.columns)
df = df.join(one_hot)

one_hot = pd.get_dummies(df['Diagnosis code'])
...
one_hot = pd.get_dummies(df['Specialism code'])

df = df.drop('Specialism code',axis = 1)
one_hot = one_hot.add_suffix('_r4')
processAttributeNames += list(one_hot.columns)
df = df.join(one_hot)

one_hot = pd.get_dummies(df['Producer code'])

df = df.drop('Producer code',axis = 1)
one_hot = one_hot.add_suffix('_r5')
df = df.join(one_hot)

one_hot = pd.get_dummies(df['Section'])

df = df.drop('Section',axis = 1)
one_hot = one_hot.add_suffix('_r6')
df = df.join(one_hot)

one_hot = pd.get_dummies(df['Specialism code.1'])

df = df.drop('Specialism code.1',axis = 1)
...
for column in df:
    if column == 'timesincelastevent' or column == 'timesincecasestart' or column == 'timesincemidnight' or column == 'event_nr' or column == 'open_cases' or column == 'Age' or column == 'Number of executions':
...
for i in range(len(df)):
    ...
    string = str(df.at[i, 'Case ID']) + " " + str(df.at[i, 'Activity code'])
...
eventAttributeNames = [attributeName for attributeName in df.columns if attributeName not in processAttributeNames + ['Case ID', 'label', 'Activity code']]
...
class TemporalGNN(torch.nn.Module):
    ...
    def forward(self, df, index, prevEventActivityNumbers, E, H):
        ...
        activityNumber = df.at[index, 'Activity code']
...
model.load_state_dict(torch.load("2011_1_with_all_seed=0_summed_loss_inter_case.pt"))
