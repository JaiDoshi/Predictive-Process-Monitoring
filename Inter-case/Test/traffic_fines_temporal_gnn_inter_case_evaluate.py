...
df = pd.read_csv("/content/drive/My Drive/Process Modelling Datasets/traffic_fines_1.csv", sep=';', dtype={'article': "string", 'Resource': "string"})
...
#Drop some columns (dropping timesincemidnight since it has only one unique value)
df = df.drop('hour', axis=1)
df = df.drop('timesincemidnight', axis=1)

processAttributeNames = ['points']

one_hot = pd.get_dummies(df['vehicleClass'])
...
one_hot = pd.get_dummies(df['article'])
...
one_hot = pd.get_dummies(df['lastSent'])

df = df.drop('lastSent',axis = 1)
one_hot = one_hot.add_suffix('_r4')
df = df.join(one_hot)

one_hot = pd.get_dummies(df['notificationType'])

df = df.drop('notificationType',axis = 1)
one_hot = one_hot.add_suffix('_r5')
df = df.join(one_hot)

one_hot = pd.get_dummies(df['dismissal'])

df = df.drop('dismissal',axis = 1)
...
for column in df:
    if column == 'timesincelastevent' or column == 'timesincecasestart' or column == 'timesincemidnight' or column == 'event_nr' or column == 'open_cases' or column == 'amount' or column == 'points' or column == 'expense':
        mex = df[column].max()
        mine = df[column].min()
        df[column] -= mine
        df[column]/=(mex-mine)

#Standardization of 'timesincelastevent', 'timesincecasestart' and 'open_cases'
...
df['open_cases'] = (df['open_cases'] - df['open_cases'].mean())/df['open_cases'].std()
