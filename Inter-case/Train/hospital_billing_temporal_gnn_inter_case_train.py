...
#Read dataframe
df = pd.read_csv("/content/drive/My Drive/Process Modelling Datasets/hospital_billing_3.csv", sep=';')
...
#Drop some columns (diagnosis dropped due to too many unique values (1016))
df = df.drop('hour', axis=1)
df = df.drop('diagnosis', axis=1)

processAttributeNames = []

dataset_name = 'Hospital_billing_3'

#For hospital_billing_2
def convertToOneHot2(df, processAttributeNames):
    one_hot = pd.get_dummies(df['speciality'])

    df = df.drop('speciality',axis = 1)
    ...
    one_hot = pd.get_dummies(df['actOrange'])

    df = df.drop('actOrange',axis = 1)
    one_hot = one_hot.add_suffix('_r3')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['actRed'])

    df = df.drop('actRed',axis = 1)
    one_hot = one_hot.add_suffix('_r4')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['blocked'])

    df = df.drop('blocked',axis = 1)
    one_hot = one_hot.add_suffix('_r5')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['caseType'])

    df = df.drop('caseType',axis = 1)
    one_hot = one_hot.add_suffix('_r6')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['flagC'])

    df = df.drop('flagC',axis = 1)
    one_hot = one_hot.add_suffix('_r7')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['flagD'])

    df = df.drop('flagD',axis = 1)
    one_hot = one_hot.add_suffix('_r8')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['msgCode'])

    df = df.drop('msgCode',axis = 1)
    one_hot = one_hot.add_suffix('_r9')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['msgType'])
    ...
    one_hot = pd.get_dummies(df['version'])

    df = df.drop('version',axis = 1)
    one_hot = one_hot.add_suffix('_r12')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['isCancelled'])

    df = df.drop('isCancelled',axis = 1)
    one_hot = one_hot.add_suffix('_r13')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['closeCode'])

    df = df.drop('closeCode',axis = 1)
...
def convertToOneHot3(df, processAttributeNames):
    ...
    one_hot = pd.get_dummies(df['isClosed'])

    df = df.drop('isClosed',axis = 1)
    one_hot = one_hot.add_suffix('_r14')
    df = df.join(one_hot)
...
if dataset_name == 'Hospital_billing_2':
    df = convertToOneHot2(df, processAttributeNames)
elif dataset_name == 'Hospital_billing_3':
...
for column in df:
    if column == 'timesincelastevent' or column == 'timesincecasestart' or column == 'timesincemidnight' or column == 'event_nr' or column == 'open_cases' or column == 'msgCount':
        mex = df[column].max()
        mine = df[column].min()
        df[column] -= mine
        df[column]/=(mex-mine)
