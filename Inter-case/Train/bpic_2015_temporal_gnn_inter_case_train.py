...
#Read dataframe
df = pd.read_csv("/content/drive/My Drive/Process Modelling Datasets/BPIC15_1_f2.csv", sep=';', dtype={'Responsible_actor': "string", 'monitoringResource': "string"})
...
processAttributeNames = ['SUMleges']

dataset_name = 'BPIC2015_1'

#For BPIC 2015 1
def convertToOneHot1(df, processAttributeNames):
    one_hot = pd.get_dummies(df['Responsible_actor'])

    df = df.drop('Responsible_actor',axis = 1)
    one_hot = one_hot.add_suffix('_r1')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Aanleg (Uitvoeren werk of werkzaamheid)'])

    df = df.drop('Aanleg (Uitvoeren werk of werkzaamheid)',axis = 1)
    one_hot = one_hot.add_suffix('_r2')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Bouw'])

    df = df.drop('Bouw',axis = 1)
    one_hot = one_hot.add_suffix('_r3')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Brandveilig gebruik (melding)'])

    df = df.drop('Brandveilig gebruik (melding)',axis = 1)
    one_hot = one_hot.add_suffix('_r4')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Brandveilig gebruik (vergunning)'])

    df = df.drop('Brandveilig gebruik (vergunning)',axis = 1)
    one_hot = one_hot.add_suffix('_r5')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Gebiedsbescherming'])

    df = df.drop('Gebiedsbescherming',axis = 1)
    one_hot = one_hot.add_suffix('_r6')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Handelen in strijd met regels RO'])

    df = df.drop('Handelen in strijd met regels RO',axis = 1)
    one_hot = one_hot.add_suffix('_r7')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Inrit/Uitweg'])

    df = df.drop('Inrit/Uitweg',axis = 1)
    one_hot = one_hot.add_suffix('_r8')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Kap'])

    df = df.drop('Kap',axis = 1)
    one_hot = one_hot.add_suffix('_r9')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Milieu (melding)'])

    df = df.drop('Milieu (melding)',axis = 1)
    one_hot = one_hot.add_suffix('_r10')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Milieu (neutraal wijziging)'])

    df = df.drop('Milieu (neutraal wijziging)',axis = 1)
    one_hot = one_hot.add_suffix('_r11')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Milieu (omgevingsvergunning beperkte milieutoets)'])

    df = df.drop('Milieu (omgevingsvergunning beperkte milieutoets)',axis = 1)
    one_hot = one_hot.add_suffix('_r12')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Milieu (vergunning)'])

    df = df.drop('Milieu (vergunning)',axis = 1)
    one_hot = one_hot.add_suffix('_r13')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Monument'])

    df = df.drop('Monument',axis = 1)
    one_hot = one_hot.add_suffix('_r14')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Reclame'])

    df = df.drop('Reclame',axis = 1)
    one_hot = one_hot.add_suffix('_r15')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['Sloop'])

    df = df.drop('Sloop',axis = 1)
    one_hot = one_hot.add_suffix('_r16')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['monitoringResource'])

    df = df.drop('monitoringResource',axis = 1)
    one_hot = one_hot.add_suffix('_r17')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['question'])

    df = df.drop('question',axis = 1)
    one_hot = one_hot.add_suffix('_r18')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['org:resource'])

    df = df.drop('org:resource',axis = 1)
    one_hot = one_hot.add_suffix('_r19')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['month'])

    df = df.drop('month',axis = 1)
    one_hot = one_hot.add_suffix('_r20')
    df = df.join(one_hot)

    one_hot = pd.get_dummies(df['weekday'])

    df = df.drop('weekday',axis = 1)
    one_hot = one_hot.add_suffix('_r21')
    df = df.join(one_hot)

    return df

#For BPIC 2015 2
def convertToOneHot2(df, processAttributeNames):
    one_hot = pd.get_dummies(df['Responsible_actor'])
...
#For BPIC 2015 3
def convertToOneHot3(df, processAttributeNames):
    one_hot = pd.get_dummies(df['Responsible_actor'])
    ...
    one_hot = pd.get_dummies(df['Flora en Fauna'])

    df = df.drop('Flora en Fauna',axis = 1)
    one_hot = one_hot.add_suffix('_r6')
    processAttributeNames += list(one_hot.columns)
    df = df.join(one_hot)
    ...
    df = df.drop('weekday',axis = 1)
    one_hot = one_hot.add_suffix('_r22')
    df = df.join(one_hot)

    return df

#For BPIC 2015 4
def convertToOneHot4(df, processAttributeNames):
    one_hot = pd.get_dummies(df['Responsible_actor'])
...
#For BPIC 2015 5
def convertToOneHot5(df, processAttributeNames):
    one_hot = pd.get_dummies(df['Responsible_actor'])
...
if dataset_name == 'BPIC2015_1':
    df = convertToOneHot1(df, processAttributeNames)
elif dataset_name == 'BPIC2015_2':
    df = convertToOneHot2(df, processAttributeNames)
elif dataset_name == 'BPIC2015_3':
    df = convertToOneHot3(df, processAttributeNames)
elif dataset_name == 'BPIC2015_4':
    df = convertToOneHot4(df, processAttributeNames)
elif dataset_name == 'BPIC2015_5':
    df = convertToOneHot5(df, processAttributeNames)
else:
...
for column in df:
    if column == 'timesincelastevent' or column == 'timesincecasestart' or column == 'timesincemidnight' or column == 'event_nr' or column == 'open_cases' or column == 'SUMleges':
        mex = df[column].max()
        mine = df[column].min()
        df[column] -= mine
        df[column]/=(mex-mine)

#Standardization of 'timesincelastevent', 'timesincecasestart'
...
train_model(model, optimizer, loss_func, 200, maxPrefixLength, "/content/drive/MyDrive/Temporal GNN/BPIC 2015/Ordered/2015_1_with_all_seed=0_summed_loss_inter_case.pt", train_case_ids, validation_case_ids)
